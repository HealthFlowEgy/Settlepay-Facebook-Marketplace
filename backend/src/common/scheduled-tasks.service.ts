...
export class ScheduledTasksService {
  ...
  async autoRefundExpiredEscrows() {
    const expired = await this.prisma.deal.findMany({
      where: { status: DealStatus.SHIPPED, escrowExpiresAt: { lt: new Date() } },
    });
  ...
  async autoReleaseAfterDisputeWindow() {
    const readyToRelease = await this.prisma.deal.findMany({
      where: { status: DealStatus.DELIVERY_CONFIRMED, disputeWindowEnd: { lt: new Date() } },
    });
  ...
  /** HI-07 fix: Every 5 minutes — retry PAYMENT_ERROR deals (async retry — not blocking thread) */
  @Cron('*/5 * * * *')
  async retryPaymentErrors() {
    const retryWindow = new Date(Date.now() - 5 * 60 * 1000); // Updated in last 5 min
    const errorDeals = await this.prisma.deal.findMany({
      where: { status: DealStatus.PAYMENT_ERROR, updatedAt: { gte: retryWindow } },
    });
    for (const deal of errorDeals) {
      try {
        await this.escrow.retryEscrowDeduction(deal.id);
        this.logger.log(`Retried deduction for deal ${deal.id}`);
      } catch (err: any) {
        this.logger.error(`Deduction retry failed for deal ${deal.id}: ${err.message}`);
      }
    }
  }

  /** HI-07 fix: Every 5 minutes — retry PAYOUT_FAILED deals */
  @Cron('*/5 * * * *')
  async retryPayoutFailures() {
    const retryWindow = new Date(Date.now() - 5 * 60 * 1000);
    const failedDeals = await this.prisma.deal.findMany({
      where: { status: DealStatus.PAYOUT_FAILED, updatedAt: { gte: retryWindow } },
    });
    for (const deal of failedDeals) {
      try {
        await this.escrow.retryPayout(deal.id);
        this.logger.log(`Retried payout for deal ${deal.id}`);
      } catch (err: any) {
        this.logger.error(`Payout retry failed for deal ${deal.id}: ${err.message}`);
      }
    }
  }
  ...
  async flagOverdueDisputes() {
    ...
    for (const d of overdue) {
      this.logger.error(`OVERDUE DISPUTE: ${d.id} — Deal ${d.dealId} — EGP ${d.deal.amount}`);
    }
    if (overdue.length) this.logger.warn(`${overdue.length} overdue disputes require immediate action`);
  }
